// MejorAhora - STAGING -> REGISTROS approval workflow
...
// - aprobarFila() es MANUAL: usuario hace clic en el menu sobre la fila en STAGING
// - Busca en REGISTROS la fila con mismo Numero de Credito (match exacto, tolerante a espacios)
// - Si encuentra: actualiza SU ESTADO=Realizado + borra fila de STAGING
...
function aprobarFila() {
  ...
  if (active.getName() !== 'STAGING') {
    ui.alert('Debes estar en STAGING para aprobar filas.');
    return;
  }
  ...
  if (matchRow === -1) {
    ui.alert(
      ...
      'Revisa: (1) el numero de credito en STAGING, (2) si el cliente fue eliminado de REGISTROS.'
    );
    return;
  }
  ...
  const confirm = ui.alert(
    ...
    '\ny se BORRARA la fila ' + stgRow + ' de STAGING.' +
    '\n\n¿Continuar?',
    ui.ButtonSet.YES_NO
  );
  ...
  // Borrar fila de STAGING (Opcion B)
